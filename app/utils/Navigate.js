...
export default class Navigate {
  ...
  static onDrawerClosed = () => {
    drawerOpened = false;
  }

  static onDrawerOpened = () => {
    drawerOpened = true;
  }

  constructor(navigator, drawer) {
    this.navigator = navigator;
    this.drawer    = drawer;

    this.savedInstanceStates = [];
  ...
  _hardwareBackPress = () => {
    if (this.navigator.getCurrentRoutes()[0].path == Navigate.getInitialRoute().path) {
      ...
      if (!this.isChild) {
        ...
        // this.navigator.replaceWithAnimation(route);
        // return true;
      } else {
        this.back();
        return true;
  ...
  _saveInstanceState = (path, instanceState) => {
    if (instanceState) {
      this.savedInstanceStates.push(instanceState);
    }
  };

  // TODO
  _recoverInstanceState = (path) => {
    const index = this.savedInstanceStates.findIndex((state) => {
      return state.path === path;
    });

    if (index != -1) {
      return this.savedInstanceStates.splice(index, 1);
    }

    return null;
  };

  _clearInstanceState = (path) => {
    const index = this.savedInstanceStates.findIndex((state) => {
      return state.path === path;
    });

    if (index != -1) {
      this.savedInstanceStates.splice(index, 1);
    }
  };
    ...
    if (!path) {
      ...
      if (!obj || !obj.component) {
        ...
        const route  = {
          ...
          hideNavBar: obj.hideNavBar,
          drawerUnlock: obj.drawerUnlock,
          actions: obj.actions,
          subToolbar: obj.subToolbar,
          scrollY: obj.scrollY,
          props
        };
  ...
  toWithoutAnimation = (path, title, props) => {
    if (!path) {
      console.warn(`[Navigate.to(undefined)] A route path is required to navigate to`);
    } else {
      const obj = this._getRouteObject(path);
    ...
    if (!currentObject.children || !Object.keys(currentObject.children).length) {
      ...
      if (child) {
        ...
        if (!obj) {
          ...
          obj.title           = route.title;
          currentObject.cache = ((props) && (props.cache)) ? props.cache : null;
