...
export default class ModalPicker extends BaseComponent {
  ...
  renderSection(section) {
    return (
      <View key={section.key}
        ...
        <Text
          style={[styles.sectionTextStyle,this.props.sectionTextStyle]}>{section.label}</Text>
      </View>
    );
  }
  ...
  renderOption(option) {
    return (
      <TouchableHighlight underlayColor="#f5f5f5" key={option.key} onPress={()=>this.onChange(option)}>
  ...
  renderChildren() {
    ...
    return (
      <View style={[styles.selectStyle, this.props.selectStyle]}>
        <Text
          style={[styles.selectTextStyle, this.props.selectTextStyle]}>{this.state.selected}</Text>
      </View>
    );
  }
  ...
  render() {
    ...
    return (
      <View style={this.props.style}>
        {dp}
        <TouchableOpacity activeOpacity={0.7} onPress={(!this.props.disabled) ? this.open : null}>
          {this.renderChildren()}
        </TouchableOpacity>
      </View>
    );
